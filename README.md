...
当前交付的是 `DES-1A.1` 工作台与存储基线，不是完整专业画布。节点编辑、Frame/文字/形状工具、CanvasKit 正式渲染、AI 生成设计、可执行原型和项目关联仍在后续阶段实现；界面中的相关入口保持禁用或明确标注为后续能力。

## 路线图

- 长期记忆：计划在后续阶段设计为可审计、可控制的独立能力；当前尚未作为现有能力提供。
- AI 设计工作台：`DES-1A.1` 已建立本地设计资产、不可变修订和工作台外壳；下一阶段进入 Design AST 与命令事务，专业编辑、AI 生成和设计到开发闭环继续按专项路线分期交付。
