...
当前已交付 `DES-1A.1` 工作台与存储基线、`DES-1A.2` SVG 画布基础闭环，以及 `DES-1A.3` 导航、选择和场景查询基础。空白设计会生成 `1440 × 900` 的本地模板（Frame、背景矩形和标题文本）；DesignDocument 会在打开和保存时执行归一化与校验。画布支持 Frame、矩形、文字、图片及缺失图片占位的 SVG 渲染，并支持鼠标/键盘选择、锁定节点保护、属性编辑、拖拽移动和统一 viewBox 坐标换算。导航支持以光标为锚点的滚轮缩放、空格/中键平移、键盘 `+/-/0`、适配画布和重置视图；选择支持普通点击替换、Shift/Ctrl/Cmd 追加或移除、空白区域文档坐标框选、Escape 清空，以及多选统一包围框和属性范围摘要。渲染器无关的线性 `SceneIndex` 已统一点命中、矩形命中和视口相交查询：重叠节点按视觉层级优先选择，隐藏、锁定及容器节点遵循统一过滤规则，SVG 仅渲染视口内节点，同时保留选中或拖拽节点以维持操作稳定。固定 100、1,000、10,000 节点网格基准会校验查询结果，并记录构建、median 和 P95。选择、视口和索引状态均不会修改设计节点或 revision。`Ctrl/Cmd+S` 会保存当前文档，保存前校验数据，revision 冲突或保存失败时保留本地编辑，不静默覆盖远端版本。

这仍不是完整专业画布。点命中顺序、矩形命中和 SVG 视口裁剪基础已交付，但 R-tree/四叉树等最终空间索引、真实画布万级节点帧性能门禁，以及独立画布窗口、PNG/SVG/PDF 导出、CanvasKit 正式主渲染、组件与变量、AI Composer 的真实模型调用、可执行原型、Design → Code 和项目实现关联仍按 `DES-1A.3` 后续任务及后续阶段交付；当前相关入口保持禁用或明确标注为后续能力。

## 路线图

- 长期记忆：计划在后续阶段设计为可审计、可控制的独立能力；当前尚未作为现有能力提供。
- AI 设计工作台：`DES-1A.1` 已建立本地设计资产、不可变修订和工作台外壳，`DES-1A.2` 已完成 SVG 画布基础闭环，`DES-1A.3` 已完成 viewBox 导航、选择集、渲染器无关的场景查询和 SVG 视口裁剪基础；CanvasKit、最终空间索引与帧性能门禁、组件/变量、AI 生成和设计到开发闭环继续按专项路线分期交付。
