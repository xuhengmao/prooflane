...
pub fn permission_request_body(lang: Lang) -> &'static str {
    match lang {
        Lang::ZhCn => "智能体正在请求权限，请在 Prooflane 中查看并批准。",
        Lang::ZhTw => "智慧代理正在請求權限，請在 Prooflane 中查看並批准。",
        Lang::Ja => "エージェントが権限を要求しています。Prooflane で確認して承認してください。",
        Lang::Ko => "에이전트가 권한을 요청하고 있습니다. Prooflane에서 확인하고 승인하세요.",
        Lang::Es => "Un agente solicita permiso. Revísalo y apruébalo en Prooflane.",
        Lang::De => {
            "Ein Agent fordert eine Berechtigung an. Bitte in Prooflane prüfen und genehmigen."
        }
        Lang::Fr => {
            "Un agent demande une autorisation. Vérifiez-la et approuvez-la dans Prooflane."
        }
        Lang::Pt => "Um agente está solicitando permissão. Revise e aprove no Prooflane.",
        Lang::Ar => "يطلب أحد الوكلاء إذنًا. يرجى مراجعته والموافقة عليه في Prooflane.",
        Lang::En => "An agent is requesting permission. Review and approve it in Prooflane.",
    }
}
...
pub fn question_request_body(lang: Lang) -> &'static str {
    match lang {
        Lang::ZhCn => "智能体正在向你提问，请在 Prooflane 中回答。",
        Lang::ZhTw => "智慧代理正在向你提問，請在 Prooflane 中回答。",
        Lang::Ja => "エージェントが質問しています。Prooflane で回答してください。",
        Lang::Ko => "에이전트가 질문하고 있습니다. Prooflane에서 답변하세요.",
        Lang::Es => "Un agente te hace una pregunta. Respóndela en Prooflane.",
        Lang::De => "Ein Agent stellt eine Frage. Bitte in Prooflane beantworten.",
        Lang::Fr => "Un agent vous pose une question. Répondez-y dans Prooflane.",
        Lang::Pt => "Um agente está fazendo uma pergunta. Responda no Prooflane.",
        Lang::Ar => "يطرح أحد الوكلاء سؤالاً. يرجى الإجابة عليه في Prooflane.",
        Lang::En => "An agent is asking a question. Answer it in Prooflane.",
    }
}
...
pub fn help_title(lang: Lang) -> &'static str {
    match lang {
        Lang::ZhCn => "Prooflane Bot 帮助",
        Lang::ZhTw => "Prooflane Bot 幫助",
        Lang::Ja => "Prooflane Bot ヘルプ",
        Lang::Ko => "Prooflane Bot 도움말",
        Lang::Es => "Ayuda de Prooflane Bot",
        Lang::De => "Prooflane Bot Hilfe",
        Lang::Fr => "Aide de Prooflane Bot",
        Lang::Pt => "Ajuda do Prooflane Bot",
        Lang::Ar => "مساعدة Prooflane Bot",
        Lang::En => "Prooflane Bot Help",
    }
}
...
mod brand_tests {
    ...
    #[test]
    fn user_prompts_use_prooflane_brand() {
        for lang in LANGS {
            for text in [
                permission_request_body(lang),
                question_request_body(lang),
                help_title(lang),
            ] {
                assert!(text.contains("Prooflane"), "unexpected brand text: {text}");
                assert!(!text.contains("Codeg"), "upstream brand leaked: {text}");
            }
        }
    }
}
