...
Prooflane 是基于 Codeg 0.24.0 二次开发的证据驱动 AI 工程交付工作台。它将多个智能体（Claude Code、Codex CLI、OpenCode、Gemini CLI、OpenClaw、Cline 等）统一到一个工作区中，支持会话聚合、多智能体协作、桌面安装和服务器/Docker 部署。
...
- **`prooflane`**（`tauri-runtime`，默认）：完整桌面应用，包含 Tauri 窗口管理、系统通知、自动更新等
